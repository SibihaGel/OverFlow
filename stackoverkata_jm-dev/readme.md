...
1. На странице репозитория убедитесь, что выбрана ветка **dev** (1), нажмите кнопку **Clone** (2), скопируйте ссылку (
   3).
...
1. Перейдите в свою таску с которой планируете работать, нажмите на выпадающее меню "1", проверьте название ветки "2" (
   измените название, если необходимо), создайте request "3".
...
2. Убедитесь, что в данный момент активна ваша рабочая ветка (значок ярлыка слева от имени, как у ветки my-branch на
   скриншоте).
...
В проекте есть базовые родительские интерфейсы и реализации: ReadOnlyDao (ReadOnlyDaoImpl), ReadWriteDao (
ReadWriteDaoImpl),
...
- **commentType** - тип комментария. Идентифицирует родительскую сущность, к которой был оставлен комментарий (вопрос
  или ответ);
...
Названия всех классов преобразования должны заканчиваться на «**Converter**» (например: **GroupChatConverter**) и должны
храниться в пакете **converters**.
...
5. На каждый класс тестов, написаны отдельные датасеты. В случае, если их нужно изменить, применяется п. 4. Например,
   если мы тестируем ResourseAnswerController, есть подпакет dataset/[название пакета] и тут лежат все датасеты нужные
...
5. Если ожидаемые результаты не совпадут, тест сообщит об ошибке и распечатает все параметры. В ином случае вы увидите
   сообщение о том, что тест успешно пройден.

==========================================

# Как настроить и использовать профили

### Для чего нужны профили

Чтобы не вносить свои данные, связанные с базой данных в properties, можно сделать так, чтобы с помощью конфигурации
проекта можно задавать свои значения базы данных и они применялись на определенном профиле. В данном приложении мы
реализуем профили local и dev. local будет с локальными данными для подключения к локальной базе данных. dev будет общим
для проекта.

### Как создать профиль

1. Переходим в Intellij Idea "Run -> Profile... -> Edit Configuration".
2. Добавляем новый профиль конфигурации.

![](src/main/resources/static/images/profile_tutor/PROFILE1.png)

3. Нажимаем "Modify Options" и отмечаем чекбоксом дополнительные опции, которые нам понадобятся "Environment variables".
4. Заполняем поля как на скриншоте поля Name, Main Class приложения, Active Profiles.

![](src/main/resources/static/images/profile_tutor/PROFILE5.png)

5. В "Environment variables" заполняем в виде ключ-значение доступ к вашей локальной базе данных или любые другие
   необходимые переменные. Пример для базы данных:

![](src/main/resources/static/images/profile_tutor/PROFILE4.png)

6. Чтобы начать пользоваться данными из созданной конфигурации необходимо создать дополнительный
   application-local.properties. Вы можете создавать дополнительные профили, соблюдая простое правило, название должно
   быть application-{название_профиля}.properties.
7. Пример для application-local.properties для скриншота из 5 пункта:

```
#Data source for local profile
...
8. Запускаем, выбрав нужный профиль в VM Option.
   
`-ea -Dspring.profiles.active=local/dev`

![](src/main/resources/static/images/profile_tutor/vm-options.png)


==========================================

## Тестирование своих api через POSTMAN с использованием токена аутентификации

В проекте реализована аутентификация с использованием JWT токена.
Токен JWT используется для аутентификации и авторизации запросов к API сервисам.

Для получения токена необходимо отправить POST запрос на адрес /api/auth/token.
В ответе на запрос будет получен JSON объект с токеном.  
Далее, полученный токен должен быть вставлен в заголовок запросов, отправляемых с помощью клиента POSTMAN.

### Порядок действий:

1. Отправка POST запроса для получения токена:

   - Адрес: /api/auth/token
   - Тело запроса: JSON объект с полями "login" и "pass". Например:
```
   {
      "login": "your_login",
      "pass": "your_password"
...
Ответом на запрос будет JSON c ролью пользователя и токеном. Например:
```
   {
      "role": "ROLE_USER",
      "token": "eyJhbGci........"
...
```
![](src/main/resources/static/images/jwt/postman_getJwt.png)



2. Добавление полученного токена в свои запросы, используя POSTMAN

   - На вкладке Authorization выбрать пункт Bearer Token.
   - Прописать полученный токен в поле Token
   - Отправить запрос.

![](src/main/resources/static/images/jwt/postman_jwt.png)


## Важно!
Время жизни токена ограничено!

Если токен просрочен, то вы получите ответ со статусом 500 и io.jsonwebtoken.ExpiredJwtException.

После этого нужно получать токен заново.

Время жизни в милисекундах можно задать в properties:
```
   jwt.token.expired = 300000
```
